...
numactl="/users/$USER/numactl-2.0.19/numactl"
mkdir -p $RESULTS
...
for size in "${TEST_SIZES[@]}"; do
    ...
    for pattern in "${ACCESS_PATTERNS[@]}"; do
        ...
        # Make sure to set weights in /sys/kernel/mm/mempolicy/weighted_interleave/ beforehand.
        # Node0: 2; Node1: 1
        echo "    - Weighted Interleave across all nodes"
        run_with_counters ${RESULTS}/Test3/wt_interleave_all_${size}MB_${pattern}.txt \
            $numactl --weighted-interleave=all --cpunodebind=0 $BENCHMARK $size $pattern
